...
logging.basicConfig(filename="skurt-challenge.log", level=logging.DEBUG,
                    format=FORMAT)
...
def notify(subj, plain, html):
    ...
    try:
        ...
        server.sendmail(email_config.sender, email_config.recipients,
                        msg.as_string())
        server.close()
...
def insideRange(pos, poly):
    ...
    for i in range(0, len(poly) + 1):
        ...
        if (pos[1] > min(a[1], b[1]) and pos[1] <= max(a[1], b[1]) and
            pos[0] <= max(a[0], b[0])):
...
def polling(carIDs, defSleepTime=120, newDataSleepTime=10):
    ...
    while True:
        try:
            ...
            i += 1
        # If requests raises an exception send an warning email
        # as something may be wrong with the location API
